...
contract Token_ is Ownable_,  ERC20 {
    ...
    function burn(address _from, uint256 _amount) public onlyOwner returns (bool success) {
      require(_from != address(0), "Can't burn from zero address");
      uint256 accountBalance = balances[_from];
      require(accountBalance >= _amount, "Burn amount can't exceed balance");
      totalSupply -= _amount;
      balances[_from] -= _amount;
      emit Transfer(_from, address(0), _amount); // burn
      return true;
    }
